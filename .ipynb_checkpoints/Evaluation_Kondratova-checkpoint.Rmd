...
fasterq-dump --split-files -p SRR10390685 --outdir FASTQ
# Variant:
#srun --cpus-per-task=6 fasterq-dump --split-files -p SRR10390685 --outdir FASTQ
# did not worked from JupiterHub terminal, but I have found another variant 
#on sra-tool portal and tried it
#
```
...
fastqc FASTQ/SRR10390685_1.fastq -o QC/ -t 8
fastqc FASTQ/SRR10390685_2.fastq -o QC/ -t 8

## did not use srun because of the JupiterHub problems
...
Ces paramètres ont permis de conserver <code>13554096 </code> reads pairés, soit une perte de <code> 95,90992427882319</code>% des reads bruts.
...
module load samtools
 ...
 srun --cpus-per-task=8 samtools view --threads 8 SRR10390685_on_ASM904v1.sam -b > SRR10390685_on_ASM904v1.bam
...
zgrep trmNF  GCF_000009045.1_ASM904v1_genomic.gff.gz  | awk '$3=="gene"' > trmNF.gff3  
